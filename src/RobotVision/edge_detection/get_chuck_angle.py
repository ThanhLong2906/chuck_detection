...
def find_triplets_120_degrees(lines, angle_tolerance=2.0 ,tolerance=5.0):
    ...
    for i, line in enumerate(lines):
        ...
        for j, other_line in enumerate(lines):
            if i == j: continue
            # Kiểm tra sai số góc song song
            diff = abs(line['angle'] - other_line['angle'])
            if diff <= angle_tolerance or abs(diff - 360) <= angle_tolerance:
                has_parallel = True
                break
...
def calculate_oriented_angle(M, O, N):
    # Tọa độ: M(x1, y1), O(x2, y2), N(a, b)
    x1, y1 = M
    x2, y2 = O
    a, b = N

    # 1. Tính bình phương khoảng cách từ N đến M và O 
    # (Không cần căn bậc hai để tối ưu tốc độ)
    dist_NM_sq = (x1 - a)**2 + (y1 - b)**2
    dist_NO_sq = (x2 - a)**2 + (y2 - b)**2
    ...
    # 3. Tạo vector từ điểm đầu đến điểm cuối
    dx = end_pt[0] - start_pt[0]
    dy = end_pt[1] - start_pt[1]

    # 4. Tính góc bằng atan2
...
def project_point_to_line(M, N, O):
    a, b = M
    x1, y1 = N
    x2, y2 = O
    dx = x2 - x1
    ...
    t = ((a - x1) * dx + (b - y1) * dy) / (dx * dx + dy * dy)
    px = x1 + t * dx
    py = y1 + t * dy
    return px, py

def get_chuck_angle(image_rectified, cfg, save=None, mode: Literal['canny', 'lsd'] = 'canny', debug=False):
    """
